...
export const getPurposes = async (): Promise<ApiResponse<Purpose[]>> => {
  const response = await apiClient.get<ApiResponse<Purpose[]>>("/api/purposes");
...
export const getPurposesByWorkTypeId = async (workTypeId: string): Promise<ApiResponse<Purpose[]>> => {
  const response = await apiClient.get<ApiResponse<Purpose[]>>(`/api/purposes/work-type/${workTypeId}`);
  return response.data;
};

// Fetch purpose by ID
export const getPurposeById = async (id: string): Promise<ApiResponse<Purpose>> => {
  const response = await apiClient.get<ApiResponse<Purpose>>(`/api/purposes/${id}`);
  return response.data;
};

// Create a new purpose
export const createPurpose = async (purposeData: Partial<Purpose>): Promise<ApiResponse<Purpose>> => {
  const response = await apiClient.post<ApiResponse<Purpose>>("/api/purposes", purposeData);
  return response.data;
};

// Update a purpose
export const updatePurpose = async (id: string, purposeData: Partial<Purpose>): Promise<ApiResponse<Purpose>> => {
  const response = await apiClient.put<ApiResponse<Purpose>>(`/api/purposes/${id}`, purposeData);
  return response.data;
};

// Delete a purpose
export const deletePurpose = async (id: string): Promise<ApiResponse<boolean>> => {
  const response = await apiClient.delete<ApiResponse<boolean>>(`/api/purposes/${id}`);
  return response.data;
};
